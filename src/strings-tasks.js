...
function removeFirstOccurrences(str, value) {
  ...
  return index >= 0
    ? `${
        ...
        str.substring(index + value.length, str.length)
      }`
    : str;
}
...
function removeLastOccurrences(str, value) {
  ...
  return lastIndex >= 0
    ? `${
        ...
        str.substring(lastIndex + value.length, str.length)
      }`
    : str;
}
...
function sumOfCodes(str = '') {
  return str ? [...str].reduce((acc, item) => acc + item.charCodeAt(), 0) : 0;
}
...
function formatTime(minutes, seconds) {
  ...
  return `${newMin.padStart(2, 0)}:${newSec.padStart(2, 0)}`;
}
...
function isPalindrome(str) {
  const reg = /[\W_]/g;
  const firstStep = str.toLowerCase().replace(reg, '');
  const secondStep = firstStep.split('').reverse().join('');
...
function findLongestWord(sentence) {
  ...
  return arrFromSentence.sort((a, b) => b.length - a.length)[0];
}
...
function invertCase(str) {
  const toUpper = str.toUpperCase().split('');
  for (let i = 0; i < str.length; i += 1) {
    if (toUpper[i] === str[i]) {
      toUpper[i] = toUpper[i].toLowerCase();
    }
  }
  return toUpper.join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === ' ') {
      ...
      result += ROT13[normalABC.indexOf(str[i])];
    }
  }
  return result;
}
